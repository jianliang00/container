...
public enum KubeProxyPFRenderer {
    ...
    public static func renderAnchor(
        ...
        for rule in ruleSet.rules {
            ...
            lines.append(
                "nat on \(egressInterface) inet proto \(rule.protocolName.pfName) from \(vmnetInterface):network to <\(tableName)> port \(backendPort) -> (\(egressInterface))"
            )
        }
...
public struct KubeProxyPFRuleApplier: KubeProxyRuleApplying {
    ...
    public func apply(_ ruleSet: KubeProxyRuleSet) throws {
        ...
        let anchor = KubeProxyPFRenderer.renderAnchor(
            ruleSet: ruleSet,
            anchorName: config.anchorName,
            egressInterface: config.resolvedEgressInterface,
            vmnetInterface: config.resolvedVmnetInterface
        )
    ...
    private func pfConfigWithAnchorReferences(content: String, anchorURL: URL) -> String {
        ...
        if !content.contains(natAnchor) {
            let insertionIndex =
                lines.firstIndex { line in
                    line.starts(with: "rdr-anchor") || line.starts(with: "nat-anchor")
