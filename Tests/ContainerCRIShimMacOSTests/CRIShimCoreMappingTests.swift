...
struct CRIShimCoreMappingTests {
    ...
    func mapsMachineStateAnnotationsIntoSandboxConfiguration() throws {
        ...
        #expect(options.machineState?.storageDirectory == criCanonicalizedManagedDirectoryPath(storageRoot.appendingPathComponent("pod-uid-a").path))
        #expect(options.machineState?.controlSocketPath == criCanonicalizedManagedDirectoryPath(socketRoot.appendingPathComponent("pod-uid-a.sock").path))
