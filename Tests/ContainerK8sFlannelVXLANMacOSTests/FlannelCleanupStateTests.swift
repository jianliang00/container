...
struct FlannelCleanupStateTests {
    ...
    func persistsCanonicalOwnedStateAndRemovesItIdempotently() throws {
        ...
        try store.save(
            FlannelOwnershipState(
                ...
                localPodCIDR: "10.244.22.19/24",
                routePodCIDRs: ["10.244.5.9/24", "10.244.2.0/24", "10.244.5.0/24"]
            ))

        let loadedState = try store.load()
        let state = try #require(loadedState)
        #expect(state.localPodCIDR == "10.244.22.0/24")
        #expect(state.routePodCIDRs == ["10.244.2.0/24", "10.244.5.0/24"])
        ...
        #expect(throws: FlannelVXLANError.self) {
            try store.save(
                FlannelOwnershipState(
                    interfaceName: "utun42",
                    localPodCIDR: "10.244.22.0/24",
                    routePodCIDRs: ["10.244.22.128/25"]
                ))
        }
    }
    ...
    func persistsCanonicalHostOnlyNetworkOwnership() throws {
        ...
        let loadedState = try store.load()
        let state = try #require(loadedState)
        #expect(state.podCIDR == "10.244.22.0/24")
