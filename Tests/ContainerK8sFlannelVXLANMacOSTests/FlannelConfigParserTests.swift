...
struct FlannelConfigParserTests {
    ...
    func parsesWindowsCompatibleVXLANConfiguration() throws {
        let config = try FlannelConfigParser.parse(
            #"{"Network":"10.244.0.0/16","EnableIPv4":true,"Backend":{"Type":"vxlan","VNI":4096,"Port":4789,"MTU":1500}}"#
        )

        #expect(config.network == "10.244.0.0/16")
        ...
        let configMap = FlannelConfigMap(
            ...
            data: [
                "cluster-network.json":
                    #"{"Network":"10.244.7.19/16","EnableIPv6":true,"Backend":{"VNI":4096,"Port":4789}}"#
            ]
        )
    ...
    func validatesWindowsRestrictionsSeparatelyFromGenericParsing() throws {
        let config = try FlannelConfigParser.parse(
            #"{"Network":"10.244.0.0/16","Backend":{"Type":"vxlan","VNI":4097,"Port":4789}}"#
        )
