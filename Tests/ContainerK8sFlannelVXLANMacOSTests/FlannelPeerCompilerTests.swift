...
struct FlannelPeerCompilerTests {
    ...
    func compilesLinuxAndWindowsPeersUsingDynamicAnnotationPrefix() throws {
        ...
        let nodes = [
            makeNode(
                name: "mac-a",
                podCIDRs: ["fd00::/64", "10.244.25.0/24"],
                internalIP: "10.31.252.9",
                operatingSystem: "darwin"
            ),
            try makeManagedNode(
                name: "linux-a",
                podCIDR: "10.244.2.0/24",
                publicIP: "10.185.55.20",
        ...
        #expect(result.localNetwork?.podCIDR == "10.244.25.0/24")
        #expect(result.localNetwork?.subnetBase == "10.244.25.0")
        #expect(result.localNetwork?.internalIP == "10.31.252.9")
    ...
    func treatsWindowsEmptyVTEPMACAsPending() throws {
        let nodes = [
            makeNode(name: "mac-a", podCIDRs: ["10.244.25.0/24"]),
            try makeManagedNode(
                name: "windows-a",
                podCIDR: "10.244.8.0/24",
                publicIP: "10.19.121.140",
    ...
    func ignoresNodesNotManagedByFlannel() throws {
        let result = try FlannelPeerCompiler.compile(
            nodes: [
                makeNode(name: "mac-a", podCIDRs: ["10.244.25.0/24"]),
                makeNode(name: "unmanaged-mac", podCIDRs: ["10.244.24.0/24"]),
            ],
            localNodeName: "mac-a",
            networkConfig: makeNetworkConfig()
        )
    ...
    func rejectsVNIAndPodCIDRConflicts() throws {
        let nodes = [
            ...
            try makeManagedNode(
                ...
                publicIP: "10.185.55.6",
                vtepMAC: "02:00:00:00:00:06"
            ),
            try makeManagedNode(
                name: "overlap-b",
                podCIDR: "10.244.6.128/25",
                publicIP: "10.185.55.7",
    ...
    func decodesNodeAPIShapeUsedByCompiler() throws {
        let data = Data(
            #"{"items":[{"metadata":{"name":"linux-a","labels":{"kubernetes.io/os":"linux"},"annotations":{"flannel.alpha.coreos.com/kube-subnet-manager":"true"}},"spec":{"podCIDR":"10.244.2.0/24","podCIDRs":["10.244.2.0/24","fd00:2::/64"]},"status":{"addresses":[{"type":"InternalIP","address":"10.185.55.20"}],"conditions":[{"type":"Ready","status":"True"}]}}]}"#
                .utf8
        )
        ...
        #expect(list.items.first?.status?.internalIPv4 == "10.185.55.20")
