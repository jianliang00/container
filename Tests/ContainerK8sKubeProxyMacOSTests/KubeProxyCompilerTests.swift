...
struct KubeProxyCompilerTests {
    ...
    func rendersDeterministicPFAnchor() throws {
        ...
        let anchor = try KubeProxyPFRenderer.renderAnchor(ruleSet: ruleSet, localPodCIDR: "10.244.25.0/24")

        #expect(anchor.contains("# generation: 7"))
        #expect(anchor.contains("# local PodCIDR: 10.244.25.0/24"))
    ...
    func applierInstallsPFConfigAndAnchorAfterValidation() throws {
        ...
        try applier.apply(ruleSet, localPodCIDR: "10.244.25.0/24")
    ...
    func controllerUsesCanonicalRuntimePodCIDRInsteadOfStaticCIDR() async throws {
        ...
        let paths = try writePodNetworkStates(
            in: directory,
            runtimePodCIDR: "10.244.25.42/24",
            readyPodCIDR: "10.244.25.0/24"
        )
        ...
        #expect(applier.appliedPodCIDRs == ["10.244.25.0/24"])
