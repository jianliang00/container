...
struct NetworkPolicyCompilerTests {
    ...
    func compilesSimpleIngressPolicyForSelectedEndpoint() throws {
        ...
        #expect(apiPolicy.ingressACL[0].peer == .ipv4Host(try IPv4Address("10.250.0.10")))
    ...
    private func makeEndpoints() throws -> [EndpointIdentity] {
        [
            EndpointIdentity(
                ...
                ipv4Address: try IPv4Address("10.250.0.10"),
            ...
            EndpointIdentity(
                ...
                ipv4Address: try IPv4Address("10.250.0.20"),
                labels: ["role": "api"],
                namespaceLabels: ["name": "app"]
            ),
            EndpointIdentity(
                ...
                ipv4Address: try IPv4Address("10.250.1.30"),
                labels: ["role": "frontend"],
                namespaceLabels: ["name": "app"]
            ),
        ]
