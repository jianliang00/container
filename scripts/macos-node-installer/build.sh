...
stage_file "${BUILD_BIN_DIR}/container-macos-node-status" "${PKGROOT}/usr/local/bin/container-macos-node-status" 0755
...
if [[ "$SKIP_CODESIGN" != true ]]; then
    ...
    codesign_path "${PKGROOT}/usr/local/bin/container-macos-node-status" --prefix=com.apple.container.
